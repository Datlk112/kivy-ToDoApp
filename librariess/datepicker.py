...
class BaseDialogPicker(
    ...
    selector_color = ColorProperty("purple")
    """
    ...
    text_current_color = ColorProperty("purple")
    """
    ...
    text_button_color = ColorProperty("purple")
    """
...
class MDDatePicker(BaseDialogPicker):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.max_date and self.min_date:
            if self.min_date and not isinstance(self.min_date, JalaliDate):
                raise DatePickerTypeDateError(
                    "'min_date' must be of class <class 'datetime.date'>"
                )
            if self.max_date and not isinstance(self.max_date, JalaliDate):
                raise DatePickerTypeDateError(
    ...
    def on_ok_button_pressed(self) -> None:
        ...
        self.dispatch(
            "on_save",
            JalaliDate(self.sel_year, self.sel_month, self.sel_day),
            self.get_date_range(),
        )
    ...
    def set_selected_widget(self, widget) -> None:
        if self._select_year_dialog_open or self._input_date_dialog_open:
            return
        try:
            widget_date = JalaliDate(self.year, self.month, int(widget.text))
        except ValueError:
            return
        if self.mode == "picker":
